...
DECISION_PREFIX = "AOA-KAG-D"
DECISION_RE = re.compile(r"^AOA-KAG-D-(\d{4})-[a-z0-9][a-z0-9-]*\.md$")
DECISION_ID_RE = re.compile(r"^AOA-KAG-D-\d{4}$")
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob("*.md")):
        ...
        if not path.name.startswith(f"{DECISION_PREFIX}-"):
            issues.append((rel, "top-level decision markdown must use AOA-KAG-D-####-slug.md"))

    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        rel = path.relative_to(repo_root).as_posix()
        match = DECISION_RE.match(path.name)
        if not match:
            issues.append((rel, "filename must match AOA-KAG-D-####-slug.md"))
            continue
        ...
        kag_surfaces = _metadata_value(metadata, "KAG surfaces", rel, issues)
        ...
        if decision_id and not DECISION_ID_RE.match(decision_id):
            issues.append((rel, "Decision ID must match AOA-KAG-D-####"))
        if original_date:
            try:
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    kag_surfaces=_split_values(kag_surfaces),
    ...
    if numbers:
        ...
        if numbers != expected_numbers:
            issues.append((DECISIONS_DIR.as_posix(), "decision numbers must be contiguous from AOA-KAG-D-0001"))
...
def validate_index_contract(repo_root: Path = REPO_ROOT) -> list[tuple[str, str]]:
    ...
    text = path.read_text(encoding="utf-8")
    required = [
        "schema_version: aoa_kag_decision_index_contract_v1",
        "authority: docs/decisions/AGENTS.md",
        "source_records: docs/decisions/AOA-KAG-D-*.md",
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "KAG surfaces":
                values.append(", ".join(record.kag_surfaces))
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        INDEX_DIR / "README.md": (
            ...
            "- [By KAG surface](by-kag-surface.md)\n"
        ...
        INDEX_DIR / "by-kag-surface.md": _grouped_index(
            "Decisions By KAG Surface",
            _group_by(records, "kag_surfaces"),
        ),
