...
class CandidateSnapshot:
    ...
    def content_identity(self) -> str:
        """Bind proof-relevant candidate content while ignoring filesystem times.

        Landing preparation rewrites generated files in the isolated candidate
        and applies their exact patch to the caller.  Those two operations
        cannot preserve identical mtimes, while all bytes, modes, hardlinks,
        xattrs, index state, directories, and nested-checkout identities must
        still match exactly.
        """
...
def prepare_landing(
    ...
    *,
    ...
    try:
        ...
        applied_snapshot: CandidateSnapshot | None = None
        sealed_snapshot: CandidateSnapshot | None = None
        if mode == "apply":
            # Restore the caller's exact index partition in the already
            # validated isolated candidate.  Its worktree retains the final
            # generated bytes, so a content identity can seal the exact result
            # that must exist after applying the derived patch to the caller.
            _restore_git_index_state(
                temporary_worktree,
                snapshot.index_mode,
                snapshot.index_bytes,
            )
            sealed_snapshot = capture_candidate_snapshot(temporary_worktree)
            if patch:
            ...
            applied_snapshot = capture_candidate_snapshot(source_root)
            if applied_snapshot.content_identity() != sealed_snapshot.content_identity():
                raise PreparationFailure(
                    "applied caller candidate differs from the validated isolated candidate",
                    failure_type="applied_candidate_seal_mismatch",
                    action_class="rollback_generated_patch",
                    details={
                        "expected_content_identity": sealed_snapshot.content_identity(),
                        "actual_content_identity": applied_snapshot.content_identity(),
                    },
                )
            applied_providers = verify_provider_identities(source_root)
            if applied_providers != final_providers:
                raise PreparationFailure(
                    "provider identity changed while sealing the applied caller candidate",
                    failure_type="provider_identity_mismatch",
                    action_class="retry_same_candidate",
                    details={
                        "before": list(final_providers),
                        "after": list(applied_providers),
                    },
                )
            require_candidate_unchanged(source_root, applied_snapshot)
