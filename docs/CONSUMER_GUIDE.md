...
1. Start with `mechanics/distillation/parts/tos-text-chunk-map/generated/tos_text_chunk_map.min.json`.
   This gives the current source-linked retrieval units for the bounded
   Zarathustra authority slice.
2. Move to `mechanics/boundary-bridge/parts/tos-retrieval-axis/generated/tos_retrieval_axis_pack.min.json`.
   This adds bounded source, lineage, conflict, practice, and memo handles
   without ranking or routing policy.
3. Read `mechanics/boundary-bridge/parts/federation-spine/generated/federation_spine.min.json`.
   This gives the current two-repo source-owned tiny export entrypoints for
   `aoa-techniques` and `Tree-of-Sophia`.
4. Read `mechanics/boundary-bridge/parts/cross-source-projection/generated/cross_source_node_projection.min.json`.
...
5. When provenance weakens or a derived surface starts sounding stronger than
   the source that feeds it, read `mechanics/recurrence/parts/return-regrounding/generated/return_regrounding_pack.min.json`.
...
`counterpart_refs` remain contract-only in the current planned scope.
...
Do not treat `mechanics/boundary-bridge/parts/counterpart-edge/examples/counterpart_edge_view.example.json` as an active
