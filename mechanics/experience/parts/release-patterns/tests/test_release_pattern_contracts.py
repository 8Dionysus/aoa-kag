...
ESCAPE_VALUE = "__release_pattern_not_allowed__"

RELEASE_PATTERN_CONTRACTS = (
    ('post_release_pattern_candidate', 'post_release_pattern_candidate_v1.json'),
    ('kag_installation_precedent_candidate_v1', 'kag_installation_precedent_candidate_v1.json'),
    ('kag_release_pattern_candidate_v1', 'kag_release_pattern_candidate_v1.json'),
)
...
class ReleasePatternContractTests(unittest.TestCase):
    ...
    def test_release_pattern_examples_match_schemas(self) -> None:
        self.assertTrue(RELEASE_PATTERN_CONTRACTS)
        missing_pairs: list[str] = []
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema_path = PART_ROOT / "schemas" / schema_file
            example_path = PART_ROOT / "examples" / f"{stem}.example.json"
        ...
        self.assertFalse(missing_pairs, "missing release-pattern contract pair(s): " + ", ".join(missing_pairs))
    ...
    def test_release_pattern_schemas_reject_unknown_fields(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_wrong_types_for_every_field(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_missing_required_fields(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_bad_array_items(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_empty_strings(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_const_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_enum_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_release_pattern_schemas_reject_numeric_bound_escapes(self) -> None:
        for stem, schema_file in RELEASE_PATTERN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
            for path, value in walk_values(example):
