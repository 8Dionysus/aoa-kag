...
ENUM_ESCAPE_VALUE = "__governance_precedent_not_allowed__"

GOVERNANCE_PRECEDENT_CONTRACTS = (
    ('governance_kag_promotion_dossier', 'governance_kag_promotion_dossier_v1.json'),
    ('governance_tos_dossier_bridge', 'governance_tos_dossier_bridge_v1.json'),
    ('policy_precedent_node_candidate', 'policy_precedent_node_candidate_governance_v1.json'),
    ('policy_precedent_node_candidate_v1', 'policy_precedent_node_candidate_v1.json'),
)
...
class GovernancePrecedentContractTests(unittest.TestCase):
    ...
    def test_governance_precedent_examples_match_schemas(self) -> None:
        self.assertTrue(GOVERNANCE_PRECEDENT_CONTRACTS)
        missing_pairs: list[str] = []
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema_path = PART_ROOT / "schemas" / schema_file
            example_path = PART_ROOT / "examples" / f"{stem}.example.json"
        ...
        self.assertFalse(missing_pairs, "missing governance-precedent contract pair(s): " + ", ".join(missing_pairs))
    ...
    def test_governance_precedent_schemas_reject_unknown_fields(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent object fields were exercised")

    def test_governance_precedent_schemas_reject_wrong_types_for_every_field(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent fields were exercised")

    def test_governance_precedent_schemas_reject_missing_required_fields(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent required fields were exercised")

    def test_governance_precedent_schemas_reject_bad_array_items(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent array fields were exercised")

    def test_governance_precedent_schemas_reject_const_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent const fields were exercised")

    def test_governance_precedent_schemas_reject_enum_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance-precedent enum fields were exercised")

    def test_governance_precedent_schemas_reject_invalid_numeric_ranges(self) -> None:
        for stem, schema_file in GOVERNANCE_PRECEDENT_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
            for path, value in walk_values(example):
