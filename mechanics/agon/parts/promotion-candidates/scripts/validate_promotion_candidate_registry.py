...
REVIEW_STAGE_LABEL = 'KAG Promotion Path'
RUNTIME_POSTURE = 'candidate_only'
EXPECTED_COUNT = 9
UNIQUE_KEY_FIELD = 'candidate_id'
...
def load_builder():
    spec = importlib.util.spec_from_file_location('_agon_promotion_candidate_builder', BUILDER)
...
def validate_source_metadata(source: dict[str, Any]) -> str | None:
    expected = {
        ...
        'review_stage_label': REVIEW_STAGE_LABEL,
...
def validate_item(item: dict[str, Any]) -> str | None:
    ...
    if item.get('review_stage') != REVIEW_STAGE:
        return f'{key} review_stage must be {REVIEW_STAGE}'
