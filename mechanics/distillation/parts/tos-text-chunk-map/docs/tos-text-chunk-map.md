...
This document records the current bounded ToS text chunk-map posture at the KAG
layer.
...
This keeps the pilot aligned with the current bounded Zarathustra tiny-entry seam
...
route-pack scope does not widen `AOA-K-0005` beyond its current source-node
chunk-map posture.
...
- `mechanics/distillation/parts/tos-text-chunk-map/examples/tos_text_chunk_map.example.json`
